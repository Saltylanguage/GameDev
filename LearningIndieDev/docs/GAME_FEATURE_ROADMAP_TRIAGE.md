...
| 2 | **Phase decision moment** | At a frozen boundary the player can read what happened, take one of three Mutations, Skip, End, or Continue with confidence. | **Now** | M1 closeout / active S3 | Six-round loop; five choices after rounds 1–5; three distinct free Mutation choices from the bounded bridge pool; evidence-backed qualitative meaning. | Too many choices, weak information hierarchy, launch-only effects appearing as dead options. | Players choose intentionally and can predict the direction of change on the next round. |
...
| 6 | **Economy and currency loop** | The player understands what survival, risk and choices earn and what funds a species' permanent Genome. S3 Mutations are free choices; any later Mutation cost or Skip bonus is a separate economy decision. | **Now → Next** | Define with the expedition loop; implement with the home-base shell | Phase awards, expedition settlement, banked scientific data, Genome costs and accomplishment rules; any later Mutation spending rule requires its own approval. | Reward inflation, weak currency sinks, accidental mixing of temporary and permanent spending, or a reward rhythm that makes choices feel cosmetic. | Players can account for what they earned, choose a meaningful spend, and see why another expedition or species is worth playing. |
...
| Cause-and-effect language | 2, 5, 7–11, 14–15, 20 | Phase/final Stat-Line meaning is approved for the bounded EX-010 contract. | Choose the small set of approved facts the player must see and the deeper facts kept for the Lab; review any new metric meaning separately. |
| Visual readability | 2–5, 7–15, 18 | The desktop/Simulation composition passed graphics acceptance at both target resolutions; feature-specific readability remains active. | Improve board hierarchy, role recognition, danger, selection, and upgrade feedback without reopening the completed composition gate. |
