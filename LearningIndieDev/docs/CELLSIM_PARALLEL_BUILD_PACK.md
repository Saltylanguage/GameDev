...
| [CellSim.ps1](../CellSim.ps1) | At the original 2026-09-09 inspection, this wrapper defaulted to `legacy-fixed-damage` while the direct experiment script defaulted to `opposed-roll`. As of 2026-09-18, the wrappers and simulation runner default to `opposed-roll`; legacy fixed damage remains an explicit compatibility/replay mode. | Resolve and record every scientific setting explicitly. Pass `-CombatMode legacy-fixed-damage` only when reproducing a historical run. |
