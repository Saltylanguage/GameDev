...
**Status:** Bounded launch-time result accepted; sequential-continuation follow-up EX-010 completed and accepted
...
starts. The intended player flow allows upgrades to be acquired between
simulation segments and then continue from the current state. That timing/state
question is covered separately by EX-010.
