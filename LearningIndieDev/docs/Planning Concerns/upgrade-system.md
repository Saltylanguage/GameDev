...
- **Smallest mitigation:** Preserve authored acquisition order; define scope, stacking, exclusion, and prerequisite behavior explicitly; record order in the snapshot, fingerprint, and report; run paired A→B, B→A, and all relevant order variants in research tests rather than assuming equivalence.
