...
namespace SaltyGame
{
    public sealed class GameHud : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            transition ??= new GUIStyle(GUI.skin.label) { fontSize = 32, fontStyle = FontStyle.Bold, alignment = TextAnchor.MiddleCenter, normal = { textColor = Color.white } };

            GUI.Label(new Rect(24, 18, 500, 45), "Island Chores", title);
            GUI.Label(new Rect(26, 60, 650, 30), $"Day {runtime.Clock.Day} - {runtime.Clock.TimeOfDay} | Wood: {runtime.Inventory.Get(ResourceId.Wood)} | Berries: {runtime.Inventory.Get(ResourceId.Berries)} | Stone: {runtime.Inventory.Get(ResourceId.Stone)}", body);

            if (runtime.HasTimeMessage)
            {
                GUI.Box(new Rect(Screen.width / 2 - 190, 90, 380, 100), GUIContent.none);
                GUI.Label(new Rect(Screen.width / 2 - 180, 95, 360, 90), runtime.TimeMessage, transition);
            }
