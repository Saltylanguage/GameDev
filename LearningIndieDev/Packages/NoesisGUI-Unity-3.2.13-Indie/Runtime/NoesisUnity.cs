...
public class NoesisUnity
{
    ...
    public static event System.Action DomainUnload;
        ...
        if (!_initialized)
        {
            ...
            if (!_initialized)
            {
                ...
                System.AppDomain.CurrentDomain.DomainUnload += (sender, e) =>
                {
                    ...
                    // Shutdown Noesis to capture leak information if the editor is quitting
                    // Note: DomainUnload is not called in standalone builds
                    Noesis.GUI.Shutdown(_quitting);
                };
    ...
    public static void ReloadApplicationResources()
    {
        if (_initialized)
        {
            ...
            if (File.Exists(filename))
            {
                try
                {
                    hashStr = File.ReadAllText(filename);
                }
            ...
            if (hash.ToString() != hashStr)
            {
              #if DEBUG_IMPORTER
                Debug.Log($"=> New Global Resources: {hash}");
              #endif

                try
                {
                    File.WriteAllText(filename, hash.ToString());
                }
                catch (System.Exception e)
                {
                    Debug.LogException(e);
    ...
    {
        Noesis_MuteLog(false);
    }
    #endregion

    #region Error management
    public delegate void ErrorCallback([MarshalAs(UnmanagedType.LPUTF8Str)]string message);

    public static void PushErrorHandler(ErrorCallback errorCallback)
    {
        Noesis_PushErrorHandler(errorCallback);
    }
    public static void PopErrorHandler()
    {
        Noesis_PopErrorHandler();
    }
    ...
    static extern void Noesis_RegisterLog();

    [DllImport(Library.Name)]
    static extern void Noesis_SetLogLevel(int generalLogLevel, int bindingLogLevel);

    [DllImport(Library.Name)]
    static extern void Noesis_MuteLog(bool muted);

    [DllImport(Noesis.Library.Name)]
    static extern void Noesis_PushErrorHandler(ErrorCallback errorCallback);

    [DllImport(Noesis.Library.Name)]
    static extern void Noesis_PopErrorHandler();
