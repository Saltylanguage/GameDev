...
#region High Definition Render Pipeline
    ...
    static class RegisterNoesisCustomPass
    {
        ...
        static void RegisterCustomPass(CustomPassInjectionPoint injectionPoint)
        {
            ...
            {
                // Disable auto-clear to prevent side effects if the pass is not used by the current camera
                // We manually clear the Stencil if 'Clear Stenvcil Buffer' is set in the View
                clearFlags = ClearFlag.None,

                // Render directly to the Screen/Backbuffer (Overlay UI)
                targetColorBuffer = CustomPass.TargetBuffer.Camera,

                // Use a temporary 'Custom' depth buffer instead of the Camera's
                // Avoids binding Resolved Color [1x] with MSAA Depth [4x]
  ...
  #if UNITY_6000_4_OR_NEWER
    private static readonly Dictionary<EntityId, bool> _eyeTextureCache = new Dictionary<EntityId, bool>();
  #else
