...
public class NoesisPostprocessor : AssetPostprocessor
{
    ...
    static string BuildStatePath()
    {
        string projectPath = System.IO.Path.GetDirectoryName(Application.dataPath);
        return System.IO.Path.Combine(projectPath, "Library", "Noesis");
    }

    static void SaveBuildState(BuildState state)
    {
        try
        {
            string folder = BuildStatePath();
            System.IO.Directory.CreateDirectory(folder);

            string json = JsonUtility.ToJson(state);
            File.WriteAllText(Path.Combine(folder, "BuildState"), json);
        }
        catch (Exception e)
        {
    ...
    static BuildState LoadBuildState()
    {
        string folder = BuildStatePath();
        string filename = Path.Combine(folder, "BuildState");
        ...
        if (File.Exists(filename))
        {
            try
            {
                ...
                return JsonUtility.FromJson<BuildState>(json);
            }
            catch (Exception e)
            {
                Debug.LogException(e);
    ...
    static void Update()
    {
        // Wait for script compilation or updates to finish, as XAML files depend on C# classes
        ...
        // Store progress on disk to survive domain reloads
        BuildState state = LoadBuildState();

        if (!Progress.Exists(state.id))
        {
            state.id = Progress.Start("Building XAMLs", "", Progress.Options.Unmanaged);
            ...
            Progress.RegisterCancelCallback(state.id, cancelCallback);
        }

        if (state.tasks.Count != 0)
        {
            var asset = state.tasks.First();
            state.tasks.RemoveAt(0);

            try
            {
                Progress.Report(state.id, state.current, state.total, $"{asset}");
                state.current++;
                ...
                {
                  #if DEBUG_IMPORTER
                    Debug.Log($"=> Check {asset}");
                  #endif

                    NoesisUnity.ErrorCallback handler = (message) => { Debug.Log(message, xaml); };

                    try
                    {
                        NoesisUnity.PushErrorHandler(handler);
        ...
        if (state.tasks.Count == 0)
        {
            Progress.UnregisterCancelCallback(state.id);
            Progress.Remove(state.id);
            state.id = -1;
            state.current = 0;
            state.total = 0;
        ...
        if (reloadResources)
        {
            // ReloadApplicationResources() must be called after saving the state,
            // as it can trigger OnPostprocessAllAssets() and potentially modify the state
        ...
        string[] movedAssets, string[] movedFromAssetPaths, bool didDomainReload)
    {
        ...
        if (didDomainReload)
        {
            // Resume the task if it was interrupted by a domain reload
            if (state.tasks.Count != 0)
            {
                EditorApplication.update += Update;
            }
        ...
        {
            if (IsXaml(asset) && !state.tasks.Contains(asset))
            {
                if (state.tasks.Count == 0)
                {
                    EditorApplication.update += Update;
                ...
                state.tasks.Add(asset);
                state.total++;

                NoesisXamlProvider.instance.ReloadXaml(asset);
            }
        }
